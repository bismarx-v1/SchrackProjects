...
void loop() {
  ...
  static uint64_t timerValBeforePause = 0;  // Timer time val.
  ...
  Serial.println(timerValBeforePause);
...
#define ctdEnd     7
  ...
  switch(stateCurrent) {
    ...
    case ctuStart:
      ...
      CASE_ON_EXIT(buttons.clear(); timerValBeforePause = 0;)
      break;

    // Ctu running.
    case ctuRunning:
      CASE_ON_ENTER(N_TIMER_RESTART(timerMainHandle))

      // Time stuff.
      N_TIMER_READ(timerMainHandle, timerVal, TIMER_READ_DIVIDER)  // Read time.
      display.display(1, timerVal + timerValBeforePause);          // Display time.
      // Toggle the colons.
      TIMER_1HZ_UPDATE()
      colon1 = timer1Hz;
      colon2 = timer1Hz ^ 1;
      ...
      CASE_ON_EXIT(N_TIMER_STOP(timerMainHandle) colon1 = 0; colon2 = 0; buttons.clear();
                   timerValBeforePause                              = timerVal + timerValBeforePause;)
      break;
