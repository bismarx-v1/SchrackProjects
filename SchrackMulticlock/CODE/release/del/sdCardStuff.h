#include <Arduino.h>
#include "pins.h"
// A macro that just places stuff inside an if.
#define IF_SERIAL(SERIAL_FUNC)                                                                                                 \
  {                                                                                                                            \
    if(USE_SERIAL == 1) {                                                                                                      \
      SERIAL_FUNC                                                                                                              \
    }                                                                                                                          \
 ...
 */
void sdSetup(uint8_t spiSdCs) {
  SPI.begin(GPIO_SPI_CLOCK, GPIO_SPI_MISO, GPIO_SPI_MOSI, spiSdCs);
