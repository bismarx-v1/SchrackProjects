...
def bytesToInt(bytes, nOfBytes):
	...
	for index in range(nOfBytes):
		try:
			readBytes += bytes[index] << ((nOfBytes - 1 - index) * 8)
		except(IndexError):
			readBytes += 0
	return readBytes
...
def readBytes(file, nOfBytes):
	ret = 0
	readByte = file.read(nOfBytes)
	ret = int.from_bytes(readByte, "little")
	return ret
...
# header to out file
nat1bitfile.write(intToBytes(nat1bitfileSizeFullBytes, 4))	# full bytes
print(nat1bitfileSizeFullBytes)
print("WROTE: " + str(intToBytes(nat1bitfileSizeFullBytes, 4)))

nat1bitfile.write(intToBytes(nat1bitfileSizePartialByteBits, 1))	# partial byte
print(nat1bitfileSizePartialByteBits)
print("WROTE: " + str(intToBytes(nat1bitfileSizePartialByteBits, 1)))

nat1bitfile.write(intToBytes(sampleRate, 4))	# sample rate in Hz
print(sampleRate)
print("WROTE: " + str(intToBytes(sampleRate, 4)))
...
for i in range(sampledDataSize): # same as the for i2 part just before
	...
	if (readByte > threshold):
		...
		writeVal = 0
		#print(0)
	nat1bitfilePreview.write(writeVal.to_bytes(1, "little"))
...
for i in range(nat1bitfileSizeFullBytes):
	...
	nat1bitfile.write(intToBytes(writeByte, 1))  # write full byte to file - 8 samples
...
nat1bitfile.write(intToBytes(writeByte,1))  # write the partial byte to file - < 8 samples
